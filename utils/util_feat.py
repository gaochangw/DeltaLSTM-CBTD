...
def get_filters(args, df_description, plot=0, oversample_factor=1, scale_q=0):
    fbank = None
    filt_b = None
    filt_a = None
    # Get Sample Rate
    ...
    # Set Upper Frequency
    if args.freq_high is None:
        args.freq_high = sample_rate / 2 - 500

    # Get Spacing Points
    if args.feat_type == 'dafe':
        ###################################
        # Build TRI
        ###################################
        ...
        # Filter Banks
        if args.spacing == 'mel':
            hz_points = mel_space_dafe(args.freq_low, args.freq_high, args.n_filt)
        elif args.spacing == 'log':
            hz_points = log_space_dafe(args.freq_low, args.freq_high, args.n_filt)

        # Create Filters
        # Limit Hz Point between [0, sample_rate/2]
        hz_points = np.clip(hz_points, 0, sample_rate / 2)
        fbank_bin = np.floor((args.nfft + 1) * hz_points / sample_rate)
        fbank = np.zeros((args.n_filt, int(np.floor(args.nfft / 2 + 1))))

        # Plot
        if plot:
            fig, axes = plt.subplots(2, 1, figsize=(16, 10))
            freq_step = sample_rate / 2 / (args.nfft / 2)
            freq = np.arange(0, int(args.nfft / 2) + 1) * freq_step

        for m in range(1, args.n_filt + 1):
        ...
        if args.spacing == 'mel':
            hz_points = mel_space_aafe(args.freq_low, args.freq_high, args.n_filt)
        elif args.spacing == 'log':
            hz_points = log_space_aafe(args.freq_low, args.freq_high, args.n_filt)

        # Generate the array of bandwidth for band-pass filters
        if scale_q:
            low_q = args.q_factor / 4
            high_q = args.q_factor
            q_array = np.geomspace(low_q, high_q, hz_points.shape[0])
            print("Q Factors of Filters: ", q_array)
            bw_array = hz_points / q_array
        else:
            bw_array = hz_points / args.q_factor

        # Generate the filter coefficients
        filt_b = []
        filt_a = []
        for i in range(args.n_filt):
            b, a = signal.butter(N=2,  # The order of the filter
            ...
            #     print('The BPF of channel', i + 1, 'is unstable !')
